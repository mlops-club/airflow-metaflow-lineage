...
try:
    ...
    has_name_positional_arg = packaging.version.parse(GX_VERSION) >= packaging.version.Version("0.18.14")
except Exception:
...
class DataHubValidationAction(ValidationAction):
    ...
    def _run(
        ...
        try:
            ...
            expectation_suite_name = validation_result_suite.meta.get("expectation_suite_name")
            ...
            if isinstance(validation_result_suite_identifier, ValidationResultIdentifier):
    ...
    def get_assertions_with_results(
        ...
        dataPlatformInstance = DataPlatformInstance(platform=builder.make_data_platform_urn(GE_PLATFORM_NAME))
        docs_link = None
        if payload:
            # process the payload
        ...
        for result in validation_result_suite.results:
            ...
            kwargs = {k: v for k, v in expectation_config["kwargs"].items() if k != "batch_id"}

            result = result["result"]
            assertion_datasets = [d["dataset_urn"] for d in datasets]
            if len(datasets) == 1 and "column" in kwargs:
                assertion_fields = [builder.make_schema_field_urn(datasets[0]["dataset_urn"], kwargs["column"])]
            else:
            ...
            evaluation_parameters = (
                {k: convert_to_string(v) for k, v in validation_result_suite.evaluation_parameters.items() if k and v}
            ...
            actualAggValue = (
                result.get("observed_value") if isinstance(result.get("observed_value"), (int, float)) else None
            )
    ...
    def get_assertion_info(self, expectation_type, kwargs, dataset, fields, expectation_suite_name):
        ...
        if is_sql_alchemy or is_pandas:
            ...
            if isinstance(ge_batch_spec, SqlAlchemyDatasourceBatchSpec):
                ...
                dataset_urn = make_dataset_urn_from_sqlalchemy_uri(
                    ...
                    table_name,
                    self.env,
                    self.get_platform_instance(data_asset.active_batch_definition.datasource_name),
                    self.exclude_dbname,
                    self.platform_alias,
                ...
                if splitter_method is not None and splitter_method != "_split_on_whole_table":
                    ...
                    partitionSpec = PartitionSpecClass(partition=convert_to_string(batch_identifiers))
                ...
                if not self.parse_table_names_from_sql:
                    ...
                    return []
                query = data_asset.batches[batch_identifier].batch_request.runtime_parameters["query"]
                ...
                tables = [DatasetUrn.from_string(table_urn).name for table_urn in sql_parser_in_tables.in_tables]
                if sql_parser_in_tables.debug_info.table_error:
                    logger.warning(f"Sql parser failed on {query} with {sql_parser_in_tables.debug_info.table_error}")
                    tables = []

                if len(set(tables)) != 1:
                    warn("DataHubValidationAction does not support cross dataset assertions.")
                    return []
                for table in tables:
                    dataset_urn = make_dataset_urn_from_sqlalchemy_uri(
                        sqlalchemy_uri,
    ...
    def get_platform_instance(self, datasource_name):
        if self.platform_instance_map and datasource_name in self.platform_instance_map:
            ...
            warn(f"Datasource {datasource_name} is not present in platform_instance_map")
        return None
...
def make_dataset_urn_from_sqlalchemy_uri(
    ...
    if data_platform in ["redshift", "postgres"]:
        ...
        if url_instance.database is None:
            warn(f"DataHubValidationAction failed to locate database name for {data_platform}.")
            return None
        schema_name = schema_name if exclude_dbname else f"{url_instance.database}.{schema_name}"
        ...
        database_name = url_instance.database.lower() if data_platform == "snowflake" else url_instance.database
        if database_name.endswith(f"/{schema_name}"):
            database_name = database_name[: -len(f"/{schema_name}")]
        schema_name = schema_name if exclude_dbname else f"{database_name}.{schema_name}"
    ...
    if schema_name is None:
        warn(f"DataHubValidationAction failed to locate schema name for {data_platform}.")
        return None
...
def convert_to_string(var: Any) -> str:
    try:
        tmp = str(var) if isinstance(var, (str, int, float)) else json.dumps(var, cls=DecimalEncoder)
