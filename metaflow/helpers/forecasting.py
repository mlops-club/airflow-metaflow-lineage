...
def generate_seasonal_naive_forecast(predict_horizon_hours: int, up_to_as_of_datetime: pd.DataFrame) -> pd.DataFrame:
    ...
    up_to_as_of_datetime['datetime'] = pd.to_datetime(
        up_to_as_of_datetime[['year', 'month', 'day', 'hour']]
    )
    ...
    forecast_grid = forecast_df.assign(key=1).merge(
        location_df.assign(key=1), on='key'
    ).drop('key', axis=1)
